...
- **来源定位：**PDF 文字会按版面坐标重建行、栏和段落，适配常见多栏文章。回答只显示一个数字引用；点击后会跳到原文段落并高亮其位置。
...
Raid 将数据放在程序旁边的 `RaidData` 文件夹，不把项目资料堆积到 C 盘用户目录。`Data` 保存带版本号的长期项目资料，`Runtime` 保存少量运行设置，`Cache` 只存临时数据；缓存会在正常退出时自动清空，并在下次启动时补清理异常退出留下的内容。
